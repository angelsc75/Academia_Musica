...
def get_level(db: Session, level_id: int) -> Optional[Level]:
    try:
        ...
        logging.error(f"Error de base de datos al obtener el nivel: {str(e)}")
        raise HTTPException(status_code=500, detail="Error de base de datos")
    except Exception as e:
        logging.error(f"Error inesperado al obtener el nivel: {str(e)}")
...
def update_level(db: Session, level_id: int, **kwargs) -> Optional[Level]:
    try:
        ...
        db.rollback()
        logging.error(f"Error de base de datos al actualizar el nivel: {str(e)}")
        raise HTTPException(status_code=500, detail="Error de base de datos")
    except Exception as e:
        db.rollback()
        logging.error(f"Error inesperado al actualizar el nivel: {str(e)}")
        raise HTTPException(status_code=500, detail="Error inesperado")
...
def delete_level(db: Session, level_id: int) -> bool:
    try:
        ...
        db.rollback()
        logging.error(f"Error de base de datos al eliminar el nivel: {str(e)}")
        raise HTTPException(status_code=500, detail="Error de base de datos")
    except Exception as e:
        db.rollback()
        logging.error(f"Error inesperado al eliminar el nivel: {str(e)}")
        raise HTTPException(status_code=500, detail="Error inesperado")